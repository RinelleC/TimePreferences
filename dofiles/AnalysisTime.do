...
* Power utility, exponential discounting
	...
	cluster(subjectid) technique($maxtech) init(0.595 0.611 0.925 0.818 0.146 4.315, copy)
...
* Power utility, quasi-hyperbolic discounting
	...
	global discount "qh"
	ml model lf ml_rdu_discount_flex (r: choice $riskvars $timevars = $demog) ///
	(phi: $demog) (eta: $demog) (beta: $demog) ///
	(delta: $demog) (noiseRA: $hetero) (noiseDR: $hetero) if risk == 1 | time == 1, ///
	cluster(subjectid) technique($maxtech) init(0.596 0.611 0.925 0.978 0.733 0.146 4.094, copy)
	ml maximize, difficult
...
* CRRA utility, exponential discounting
	...
	(phi: $demog) (eta: $demog) (delta: $demog) ///
	(noiseRA: $hetero) (noiseDR: $hetero) if risk == 1 | time == 1, ///
	cluster(subjectid) technique($maxtech) init(0.4041356 0.6118054 0.9252319 2.268743 0.0372434 1.836531, copy)
	ml maximize, difficult
...
* CRRA utility, quasi-hyperbolic discounting
	...
	(phi: $demog) (eta: $demog) (beta: $demog) ///
	(delta: $demog) (noiseRA: $hetero) (noiseDR: $hetero) if risk == 1 | time == 1, ///
	cluster(subjectid) technique($maxtech) init(0.4034 0.6116 0.9257 0.9789 2.0822 0.0371 1.7374, copy)
	ml maximize, difficult
...
forvalues w = 1/6 {
		...
		global discount "qh"
		ml model lf ml_rdu_discount_flex (r: choice $riskvars $timevars = $demog) (phi: $demog) (eta: $demog) (beta: $demog) ///
	(delta: $demog) (noiseRA: $hetero) (noiseDR: $hetero) if risk == 1 | time == 1, ///
...
*Note that you need the program "estout" for this to run
if "$estout" == "yes" {
	cd Output
	
	estout * using "RDUDiscEstimatesUSA.tsv" ///
